...
describe('normalizeVehicle — éligibilité vendeur', () => {
  ...
  it('dealer absent → seller_unknown, pas de patch dealer', () => {
    // Cas nominal : dealer non fourni par la source
    const result = normalizeVehicle(
      { source: 'autoscout24.import', price: 30000 },
      emptyExisting,
    )
    expect(result.eligibility).toBe('seller_unknown')
...
describe('normalizeVehicle — orchestration', () => {
  ...
  it('dealer placeholder en base + vrai dealer AS24 → remplacement autorisé', () => {
    // Un dealer placeholder existant (N/A, vide) n'est pas protégé
    const result = normalizeVehicle(
      { source: 'autoscout24.dom', dealer: 'Mercedes Benz Hamburg', dealerCity: 'Hamburg' },
      { dealer: null, dealerCity: null },
    )
